...
public class DialogClearBonusCollabo : DialogBase
{
	public enum eBtn
	{
		Close = 0,
	...
	private const string APPLICATION_URL = "http://bubblen-march.tsns.jp/campaign";

	private const float buttonDifference = 100f;
	...
	public override void OnCreate()
	{
		...
		transform = base.transform.Find("window/CollaborationButton");
	...
	public void setup(string msg, OnDecideButton decideCB, OnCancelButton cancelCB, bool bAutoClose, Constant.eMoney rewardType, int rewardNum, int eventNo, int eventStageNum)
	{
		stagenum = eventStageNum;
		setup(decideCB, cancelCB, bAutoClose, rewardType, rewardNum, eventNo);
		setMessage(msg);
	}
	...
	public void setup(OnDecideButton decideCB, OnCancelButton cancelCB, bool bAutoClose, Constant.eMoney rewardType, int rewardNum, int eventNo)
	{
		...
		if (buttons_[2] != null)
		{
			...
			buttons_[2].transform.localPosition = new Vector3(100f, buttons_[2].transform.localPosition.y, buttons_[2].transform.localPosition.z);
			buttons_[1].transform.localPosition = new Vector3(-100f, buttons_[1].transform.transform.localPosition.y, buttons_[1].transform.transform.localPosition.z);
			buttons_[2].transform.Find("Label").GetComponent<UILabel>().text = MessageResource.Instance.getMessage(3713);
		}
	...
	private IEnumerator OnButton(GameObject trigger)
	{
		...
		case "CollaborationButton":
			Constant.SoundUtil.PlayDecideSE();
			Application.OpenURL("http://bubblen-march.tsns.jp/campaign");
			break;
		}
	}
