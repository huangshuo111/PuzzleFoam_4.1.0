...
public class Editor_UIButtonMessage : MonoBehaviour
{
	public enum Trigger
	{
		OnClick = 0,
	...
	private void OnHover(bool isOver)
	{
		if (base.enabled)
		{
			...
			if (isOver && trigger == Trigger.OnDown && (Input.GetMouseButton(0) || Input.GetMouseButton(1)))
			{
				Send();
			}
			mHighlighted = isOver;
	...
	private void OnPress(bool isPressed)
	{
		if (base.enabled)
		{
			if ((isPressed && (trigger == Trigger.OnPress || trigger == Trigger.OnDown)) || (!isPressed && trigger == Trigger.OnRelease))
			{
				Send();
			}
