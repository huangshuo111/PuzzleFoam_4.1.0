...
public class UICamera : MonoBehaviour
{
	...
	private void Update()
	{
		...
		current = this;
		if (Input.touchCount == 0 && useMouse && (useMouse || (useTouch && mIsEditor)))
		{
			ProcessMouse();
		}
		if (useTouch)
	...
	public void ProcessTouch(bool pressed, bool unpressed)
	{
		...
		if (pressed)
		{
			...
			if (currentTouch.pressed != null && ColliderManager.haveInstance)
			{
				ColliderManager.Instance.dontIntersect = true;
				ColliderManager.Instance.isDownNGUIObject = true;
			}
		}
		else
		{
	...
	public void onSPenDetached(bool flag)
	{
		Debug.Log("SPenDetached = " + flag);
		useMouse = flag;
	}
}
