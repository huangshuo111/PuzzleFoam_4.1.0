...
public class Part_RankingStage : PartBase
{
	...
	public const int RankingStageNo_ = 5000;
	...
	private EntangledInt mComboCount = new EntangledInt(0);

	private TweenPosition comboIn;

	private TweenPosition comboOut;

	private UILabel comboLabel;

	private Animation comboAnim;

	private EntangledInt maxComboCount = new EntangledInt(0);

	private EntangledInt BREAK_SCORE = new EntangledInt(10);

	private EntangledInt DROP_SCORE = new EntangledInt(50);

	private EntangledInt COMBO_SCORE = new EntangledInt(100);

	private EntangledInt REFLECT_SCORE = new EntangledInt(50);

	private EntangledInt SPECIAL_SCORE = new EntangledInt(100);

	private EntangledInt mTotalScore = new EntangledInt(0);

	private StageInfo.CommonInfo stageInfo;

	private RankingStageInfo rankingStageData;

	private RankingStageInfo.TimeInfo timeInfo;
	...
	private EntangledInt prevScore_ = new EntangledInt(0);

	private EntangledInt nowScore_ = new EntangledInt(0);

	private EntangledInt bonusScore_ = new EntangledInt(0);

	private float bonusRate_ = 1f;
	...
	private EntangledInt comboBonusCoin = new EntangledInt(0);
	...
	private int comboCount
	{
		get
		{
			return mComboCount.Value;
		}
		set
		{
			mComboCount.Value = Mathf.Min(value, Constant.ComboMax);
			if (mComboCount.Value > maxComboCount.Value)
			{
				maxComboCount.Value = mComboCount.Value;
			}
		}
	}
	...
	public int totalScore
	{
		get
		{
			return mTotalScore.Value;
		}
		set
		{
			mTotalScore.Value = Mathf.Min(value, Constant.UserScoreMax);
		}
	}
		...
		for (int i = 0; i < fieldBubbleList.Count; i++)
		{
			...
			for (int k = 0; k < fieldBubbleList.Count; k++)
			{
				if (checkedFlags[k] == 1)
				{
					...
					if (type != Bubble.eType.Fulcrum && type != Bubble.eType.RotateFulcrumL && type != Bubble.eType.RotateFulcrumR && type != Bubble.eType.Rock)
					{
						list.Add(fieldBubbleList[k]);
					}
				}
	...
	private void checkDrop(Bubble me, ref int[] checkedFlags, ref bool isDrop, bool checkFulcrum)
	{
		...
		if (checkFulcrum && (type == Bubble.eType.Fulcrum || type == Bubble.eType.RotateFulcrumL || type == Bubble.eType.RotateFulcrumR || type == Bubble.eType.Rock))
		{
			isDrop = false;
		}
		for (int i = 0; i < chainBubbleDic.Count; i++)
	...
	private IEnumerator resultSetup()
	{
		mainMenu_ = appendGlobalObj(GlobalObjectParam.eObject.MapMainUI).GetComponent<MainMenu>();
		GameObject mainMenu = GlobalRoot.Instance.getObject(GlobalObjectParam.eObject.MapMainUI);
		...
		comboBonusCoin.Value = comboBonusTbl.getComboBonusCoinNum(maxComboCount.Value);
		bonusCoin += comboBonusCoin.Value;
		otherData_ = SaveData.Instance.getGameData().getOtherData();
		if (Bridge.StageData.isRankingClear())
		{
			prevScore_.Value = Bridge.StageData.getRankingStageHighScore();
		}
		int beforeLv = Bridge.PlayerData.getLevel();
		bonusRate_ = rewardTbl.getRankingBonus(beforeLv - 1);
		bonusScore_.Value = getBonusScore(bonusRate_, totalScore);
		nowScore_.Value = totalScore + bonusScore_.Value;
		resultRewards_[1].set(Constant.eMoney.Coin, getResultCoinReward());
	...
	private IEnumerator execute()
	{
		if (bForceQuit_)
		{
			...
			partManager.requestTransition(next_part2, null, true);
			yield break;
		}
		...
		case DialogResultBase.eBtn.Retry:
			args.Add("IsRanking", true);
			args.Add("IsRetry", true);
			break;
		case DialogResultBase.eBtn.Next:
			args.Add("IsRanking", true);
		...
		yield return StartCoroutine(showChangeRanking(nowScore_.Value, prevScore_.Value));
		args.Add("IsClear", true);
		PartManager.ePart next_part = PartManager.ePart.Map;
		partManager.requestTransition(next_part, args, true);
	}

	private float GetBonusRate(int lv)
	{
		return 1f;
	}
	...
	{
		dialogManager.addActiveDialogList(DialogManager.eDialog.ClearRanking);
		DialogRankingResultClear dialog = dialogManager.getDialog(DialogManager.eDialog.ClearRanking) as DialogRankingResultClear;
		yield return StartCoroutine(dialog.show(comboBonusCoin: comboBonusCoin.Value, lv: resultData_.beforeLv, score: nowScore_.Value, highScore: prevScore_.Value, rewards: resultRewards_, exp: resultData_.addExp, bonusScore: bonusScore_.Value, bonusCoin: bonusCoin, maxCombo: maxComboCount.Value, bonusRate: bonusRate_));
		if (nowScore_.Value > prevScore_.Value)
		{
		...
		DialogRankingResultClear resultDialog = dialogManager.getDialog(DialogManager.eDialog.ClearRanking) as DialogRankingResultClear;
		Debug.Log("resultDialog = " + resultDialog);
	...
	private float getCoinRate()
	{
		...
		return getGainRate(component.getCoinRate(), component);
	}

	private int calcCoin(int coin)
	{
		return Mathf.RoundToInt((float)coin * getCoinRate());
	}

	private int getResultCoinReward()
	{
		int coin = stageInfo.Coin;
		return Mathf.RoundToInt((float)coin * getCoinRate());
	}

	private float getGainRate(RewardInfo.Rate rateData, RewardDataTable rewardTbl)
	{
		return 1f;
	}

	private int getBonusScore(float bonusRate, int score)
	{
		int num = (int)Mathf.Ceil((float)score * bonusRate / 100f);
		if (num < 10)
		{
			return 0;
		}
		float f = (float)num / 10f;
		return Mathf.RoundToInt(f) * 10;
	}

	private IEnumerator showChangeRanking(int nowScore, int prevScore)
	{
		int myRank = -1;
		int rivalRank = -1;
		List<UserData> friendList = createFriendList();
		if (checkChangeRank(nowScore, prevScore, ref myRank, ref rivalRank, friendList))
		{
			...
			yield return StartCoroutine(dialog.showRankingStage(nowScore, DummyPlayerData.Data, friendData, myRank + 1, rivalRank + 1));
			while (dialogManager.getActiveDialogNum() > 0)
			{
				yield return 0;
			}
	...
	private List<UserData> createFriendList()
	{
		...
		if (component.getInfoCount() > userList.Count)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				UserData item = DummyPlayFriendData.createRankingDummyFriend(i);
				userList.Add(item);
			}
		}
	...
	{
		int bonusChanceLv = 6;
		if (Bridge.PlayerData.getLevel() >= bonusChanceLv)
		{
